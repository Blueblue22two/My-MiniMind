...
class CriticModel(MyMindForCausalLM):
    """
        自定义的Critic模型,在原本model的基础上添加来一个value_head
    """
...
def calculate_rewards(prompts, responses, reward_model, reward_tokenizer):
    ...
    # 使用reward model计算整个response的奖励（包括格式奖励和内容奖励）
    with torch.no_grad():
        reward_model_scores = []
    ...
    # 计算 final rewards = format_rewards + mark_rewards + reward_model_scores
    return rewards
...
def ppo_train_epoch(epoch, loader, iters, old_actor_model, ref_model, actor_scheduler, critic_scheduler, reward_model, reward_tokenizer, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # full mask: 用于区分生成的response部分和padding部分，确保后续计算中只关注有效的token
        full_mask = (gen_out != tokenizer.pad_token_id).long()  # [B, P+R]
        values_seq = critic_model(input_ids=gen_out, attention_mask=full_mask)  # [B, P+R]

        # 获取每行最后一个非pad token 索引
        last_indices = (full_mask * torch.arange(full_mask.size(1), device=gen_out.device)).argmax(dim=1) # [B]
        
        # 将最后token的value作为整条序列的value估计
        ...
        # actor的对数概率：只计算response部分，并且mask掉padding部分
        actor_logp = (logp_tokens * final_mask).sum(dim=1)  # [B]

        # 对于不需要更新的ref和old actor模型，使用torch.no_grad()避免计算图构建和梯度计算，节省内存和计算资源
        # 计算ref和old actor的对数概率
        with torch.no_grad():
