...
def logits_to_log_probs(logits, labels):
    """
        输出logits 转化为 每个位置真实token的对数概率
    """
    # logits shape: (batch_size, seq_len, vocab_size) vocab size = 6400
    # labels shape: (batch_size, seq_len)
    # log_probs shape: (batch_size, seq_len)
    log_probs = F.log_softmax(logits, dim=2) # log_softmax = 先softmax再取对数，数值更加稳定

    log_probs_per_token = torch.gather(log_probs, dim=2, index=labels.unsqueeze(2)).squeeze(-1) # squeeze将最后一维 vocab size删除
    # gather函数 将 Vocab 维度（最后一维）根据 labels 筛选出唯一的值，最后删除被压缩的最后一维
